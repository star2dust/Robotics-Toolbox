function inobst=inobstpoly2(start,goal,envir,disp_base_min,amp)
% inobstpoly2    Collision checking with rotation in 2D.
...
disp_base_goal = disp_base_inflate*rot2(goal(3)/amp)';
if max(inpolygon(goal(1)+disp_base_goal(:,1),goal(2)+disp_base_goal(:,2),obstpoly(1,:),obstpoly(2,:)))
    inobst = true;
elseif norm(goal-start)>10^-4
    pos_line = start+(0:dist:norm(goal-start))'.*unit(goal-start);
    disp_base_line = [];
    for i=1:size(pos_line,1)
        disp_base_line = [disp_base_line;disp_base_inflate*rot2(pos_line(i,3)/amp)'+pos_line(i,1:2)];
    end
