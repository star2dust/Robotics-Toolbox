function [nodez,nodezinpoly,nodegrid,edgez] = polyadd2(nodez,nodepoly,nodezinpoly,edgez,randpolynum,config,envir,robot_num,disp_base_max,disp_end_ref)
...
for i=polynum-randpolynum+1:polynum % random ploytope
    for j=1:polynum-randpolynum % exist ploytope
        [viaz,exit] = poly2nodez2([nodepoly(i),nodepoly(j)],nodez(nodezinpoly{i}(1),1:3),config,envir,robot_num,disp_base_max,disp_end_ref);
        if exit==1  
...
%%%%%
scene_lim = vec([envir.lb;envir.ub])';
set(gca,'xlim',scene_lim(1:2),'ylim',scene_lim(3:end)); 
hold on; 
for k=1:polynum
    polyplot(nodepoly(k).A,nodepoly(k).b);hold on
    plot(nodez(:,1),nodez(:,2),'bo')
    quiver(nodez(:,1), nodez(:,2), cos(nodez(:,3)), sin(nodez(:,3)),0.2,'b');
end
