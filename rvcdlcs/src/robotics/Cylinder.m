    ...
    methods (Access = private)   
        function h = createCylinder(obj,q,opt)
            ...
            h.cyl = patch('vertices',obj.vert(q), 'faces', obj.face, 'facecolor',...
                ...
                opt.facecolor, 'edgealpha', opt.edgealpha, 'parent', group);
            set(h.cyl,'Tag', [obj.name '-cylinder']);
            % plot lower surface
            vert = reshape(obj.vert(q)',6,21)';
            h.surl = patch('vertices',vert(:,1:3), 'faces', 1:21, 'facecolor',...
                opt.facecolor, 'facealpha', opt.facealpha, 'edgecolor',...
                opt.edgecolor, 'edgealpha', opt.edgealpha, 'parent', group);
            set(h.surl,'Tag', [obj.name '-lower-surface']);
            % plot upper surface
            h.suru = patch('vertices',vert(:,4:6), 'faces', 1:21, 'facecolor',...
                opt.facecolor, 'facealpha', opt.facealpha, 'edgecolor',...
                opt.edgecolor, 'edgealpha', opt.edgealpha, 'parent', group);
