...
for j = 1:n_obs
  ...
  if size(obs, 2) > 1
    ...
    hcanvas.obs(j) = patch(obs(1,k), obs(2,k), 'k', 'FaceColor', [.6,.6,.6], 'LineWidth', 0.1);
  else
    hcanvas.obs(j) = plot(obs(1,:), obs(2,:), 'ko');
  end
end
...
%         plot(pts(1,:), pts(2,:), 'm--', 'LineWidth', 1.5);
    ...
    if ~isempty(A)
        V = lcon2vert(A, b);
        k = convhull(V(:,1), V(:,2));
        hcanvas.cvx(i) = plot(V(k,1), V(k,2), 'ro-', 'LineWidth', 2);
    end
...
hcanvas.cvx(end+1) = plot([lb(1),ub(1),ub(1),lb(1),lb(1)], [lb(2),lb(2),ub(2),ub(2),lb(2)], 'k-');
