...
% intial Fc0 Fe
Fc = Fc_star+[[1,0,-1,0;
    0,1,0,-1]*0.3;pi/8,-pi/8,pi/6,-pi/6];
...
ephic = eFc(3,:);
% initial Fc_hat
Fc_hat = [Fc_star,Fc(:,2),Fc_star,Fc(:,4)];
% initial th23
...
for t=0:dt:tf
    ...
    M = L-diag(b); gamma = 5;    
    dotFc_hat(:) = kron(M,eye(3))^-1*(-gamma*kron(M,eye(3))*Fc_hat(:)-gamma*kron(b,eye(3))*Fc_star-kron(b,eye(3))*dotFc_star);
    % Fc
...
%        Uei = [eye(2),-rot2(pi/2)*SE2([0,0,Fe(3,i)]).R*epc(:,i);zeros(1,2),1];
%        Ue = blkdiag(Ue,Uei);
%     end
%     dotFc_err = dotFc_hat(:)-(Fe(:)+Re*eFc(:)-Fc_hat(:)); beta = 3; 
%     doteFc = [dotepc;zeros(1,size(epc,2))];
%     dotFe(:)=Ue*(-Re*doteFc(:)-kron(D,eye(3))*beta*sign(kron(D',eye(3))*(Fe(:)+Re*eFc(:)))+dotFc_err);
    % update
    epc(:) = epc(:)+dotepc(:)*dt;
...
%     Fe(:) = Fe(:)+dotFe(:)*dt;
    ...
    for i=1:length(eFc_SE2)
       Fe_SE2(i) = SE2(Fc(:,i))*eFc_SE2(i).inv;
