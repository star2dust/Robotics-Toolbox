...
cpm_urad = 3;
for i=1:size(cFe_bar,2)
    cpm_ul = -rot2(cphie_bar(i))*[cpm_lrad,cpm_urad;zeros(1,2)];
    cpm_ur(:,i) = max(cpm_ul,[],2);
    cpm_dl(:,i) = min(cpm_ul,[],2);
    cpm_range{i} = [cpm_ur(1,i),cpm_dl(1,i),cpm_dl(1,i),cpm_ur(1,i),cpm_ur(1,i);
    cpm_ur(2,i),cpm_ur(2,i),cpm_dl(2,i),cpm_dl(2,i),cpm_ur(2,i)];
end
...
for i=1:size(cFe_bar,2)
    lb(:,i) = [epc_dl;th2_dl;cpm_dl(:,i)];
    ub(:,i) = [epc_ur;th2_ur;cpm_ur(:,i)];
end
% intersection of hyperplanes
th2 = -pi/4;
for i=1:size(cFe_bar,2)
    cpm_cen = [cpm_ur(:,i),cpm_dl(:,i)]*ones(2,1)/2;
    q0(:,i) = [epc_cen;th2;rot2(-cphie_bar(i))*cpm_cen];
    q_intsec(:,i) = fmincon(negmupower2,q0(:,i),[],[],[],[],lb(:,i),ub(:,i),@(q) intsectcons(q,la));
end
% modify joint space to convex space => normal vector of epc+ef(q)+cpm=0
for i=1:size(cFe_bar,2)
    erotc = rot2(-cphie_bar(i));
    nx = [1;0;sin(-q_intsec(3,i))*la(1);erotc(1,:)'];
    ny = [0;1;-cos(-q_intsec(3,i))*la(1);erotc(2,:)'];
    % tangent plane
    consA{i} = [nx,ny]'; consb(:,i) = consA{i}*q_intsec(:,i);
    % initial q = [epcx,epcy,th2,epcmx,epcmy]
    q(:,i) = lsqlin(eye(length(q0(:,i))),q0(:,i),consA{i},consb(:,i),[],[],lb(:,i),ub(:,i));
    q(end-1:end,i) = rot2(cphie_bar(i))*q(end-1:end,i);
end
...
% static plot
plot(cpm_bar(1,:),cpm_bar(2,:),'g*');
for i=1:size(cFe_bar,2)
    plot(cpe_range{i}(1,:),cpe_range{i}(2,:),'r-.');
    plot(cpm_range{i}(1,:),cpm_range{i}(2,:),'g');
end
