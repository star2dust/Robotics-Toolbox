% coordinated motion given a fixed displacement
close all; clear;
...
epcl = 0.5;
epcr = 0.2;
phib = pi/4 - atan2(epmebar(2,2)+epcr,epmebar(1,2)+epcl+epcr);
% initial Fc
Fc = [0;0;0];
% initial Fb
cFb = [0;0;phib];
% Fb_SE2 = SE2(Fc)*SE2(cFb);
% Fb = SE2toF(Fb_SE2);
...
epcm_ur = -epmebar-epc_cen-epc_rad;
epcm_dl = -epmemax-epc_cen-epc_rad;
% range of th [small,big] 
th_intsec = [-1.1954,-0.2186;
         -0.3683,-0.8260];
% intial displacement
epcmhat = kron(ones(1,4),-epmebar(:,2)-epc_cen-epc_rad);
% initial cpm {small,big}
for i=1:size(epcmhat,2)
    cpmhat(:,i) = cFe_SE2(i).R*epcmhat(:,i);
end
...
h0 = patch(Fe(1,:),Fe(2,:),'y');
for i=1:size(Fe,2)
   hr{i} = robot_plot(Fe(:,i),th(:,i),xi,l,'b'); 
end
hm = plot(cpmhat(1,:),cpmhat(2,:),'go');
...
%     err_d(:) = k1*(epc(:)+f(:)+eRc*cpmhat(:));
    ...
    % calculate dot
    for i=1:size(epc,2)
        dotepc(:,i) = pt_proj(epc(:,i)+dotepcex(:,i),epc_ur,epc_dl)-epc(:,i);
        dotth23(:,i) = th_proj(th23(:,i)+dotth23ex(:,i),th_intsec(:,2))-th23(:,i);
%         dotcpmhat(:,i) = rot2(ephic(i))'*pt_proj(rot2(ephic(i))*(cpmhat(:,i)+dotcpmhatex(:,i)),epcm_ur(:,2),epcm_dl)-cpmhat(:,i);
    end
%     doteta = kron(L,eye(2))*(cpmhat(:)+dotcpmhat(:)-cpmbar{2}(:));
...
%     eta(:) = eta(:)+doteta(:)*dt;
    ...
    for i=1:length(eFc_SE2)
       ...
       Fe(:,i) = [Fe_SE2(i).t;vex(logm(Fe_SE2(i).R))];
    end
    set(h0,'XData',Fe(1,:),'YData',Fe(2,:));
    th = [Fe(3,:)-sum(th23,1);th23];
    for i=1:size(Fe,2)
        hr{i} = robot_update(hr{i},Fe(:,i),th(:,i),xi,l);
    end
    set(hm,'XData',cpmhat(1,:),'YData',cpmhat(2,:));
    pause(0.001);
end